...
> **Important Idea:** Gaining access to Wi-Fi can potentially provide an attacker with a path toward internal systems.

---

## 2. Rogue Access Point (Rogue AP)

A **Rogue Access Point (Rogue AP)** is an unauthorized wireless access point installed by an attacker.
...
A Rogue AP can potentially provide an attacker with a backdoor into the network.
...
> **Remember:** Evil Twin = Fake Wi-Fi that looks like the real Wi-Fi.

---

## 4. Disassociation / Deauthentication Attack

A **deauthentication attack** attempts to disconnect a legitimate wireless client from its real access point.
...
| Term      | Meaning                                                              |
| --------- | -------------------------------------------------------------------- |
| **SSID**  | Name or identifier of a wireless network                             |
| **BSSID** | Identifier associated with a particular wireless access point        |
| **ESSID** | In this context, essentially identifies the same network as the SSID |
...
However, a **hidden SSID does not make the network invisible**.
...
> **Key Point:** Hidden SSID ≠ Strong Security
...
A similar concept in which wireless networks are searched from:
...
An **Initialization Vector (IV)** is information used with encryption during wireless communication.
...
**WEP (Wired Equivalent Privacy)** is obsolete and insecure.
...
> **Bottom Line:** Never rely on WEP for modern wireless security.
...
> **Important:** WPA is not vulnerable to the same IV attacks as WEP.
...
> **Important:** WPA3 is not automatically immune to every possible attack.

---

## 14. WPS PIN Attack

**WPS (Wi-Fi Protected Setup)** was designed to make wireless setup easier.
...
> **Important Distinction:** A fragmentation attack does **not directly recover the WEP key**. Instead, it obtains PRGA material that can be used for further wireless attacks.
...
* **SET (Social-Engineer Toolkit)** — can assist with social engineering and credential-harvesting activities
...
> **Remember:** Bluejacking = Sending unwanted information or messages.
...
| **Bluesnarfing** | Steals or accesses information |
...
**RFID (Radio-Frequency Identification)** uses electromagnetic fields to identify and track tags containing stored information.
...
A hidden reader or skimmer may be placed near an RFID reader.
...
* **Evil Twin** → Fake Wi-Fi that imitates a legitimate network
* **Deauthentication** → Attempts to disconnect wireless clients
* **Airmon-ng** → Enables monitor mode
* **Airodump-ng** → Discovers and observes wireless networks
* **Hidden SSID** → Does not provide strong security
* **Jamming** → Creates wireless interference and DoS
* **War Driving** → Searching for Wi-Fi networks while moving around
* **WEP** → Obsolete and insecure
* **KRACK** → Key Reinstallation Attack
* **WPS PIN attack** → Exploits weak/repeated WPS PIN authentication
* **KARMA** → Uses fake APs and probe requests
* **Bluejacking** → Sends unwanted Bluetooth messages
* **Bluesnarfing** → Unauthorized access to Bluetooth data
* **Password Spraying** → One common password against many accounts
* **Credential Stuffing** → Uses previously leaked credentials
* **Exploit Chaining** → Combines multiple vulnerabilities to achieve a larger impact

> **Security Principle:** Wireless security requires more than hiding an SSID. Strong authentication, modern encryption, secure configurations, monitoring, patching, and proper access controls are all important.
