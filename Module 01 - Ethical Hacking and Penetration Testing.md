...
An ethical hacker is a security professional who thinks like an attacker to identify weaknesses in systems, networks, and applications. The purpose is to find vulnerabilities before malicious attackers can exploit them.
...
A penetration tester must always stay within the approved scope.
...
The attack surface is the collection of possible entry points that an attacker could target.
...
Other security-focused Linux distributions include:
...
Network testing tools can be used for:
...
Wireless testing tools can be used for:
...
Web application testing tools can be used for:
